...
# ---------- Display predictions like your friend ----------
print("\n===== SAMPLE PREDICTIONS =====")
for i, (true_val, pred_val, year_val) in enumerate(zip(y_valid.values[:15], preds[:15], df_model.loc[~is_train, 'Year'].values[:15])):
    print(f"Année {int(year_val)} — Vrai: {true_val:.2f}, Prédit: {pred_val:.2f}")

# ---------- Plot predictions vs true ----------
plt.figure(figsize=(8,5))
plt.scatter(y_valid, preds, alpha=0.5, color='royalblue')
...
plt.hist(residuals, bins=50, color='darkorange', alpha=0.7)

...
for species_name in species_list:
    ...
    # Tri par cellule et année pour bien décaler
    ...
    # Supprimer les premières années (pas de t-1 dispo)
    df_species = df_species.dropna(subset=["abondance_capped_t-1"])
    if df_species.empty:
        continue

    # Définition des colonnes à exclure (toutes les abondances originales + identifiants)
    ...
    # On garde la nouvelle colonne abondance_capped_t-1 comme feature
    if "abondance_capped_t-1" in df_species.columns:
        X["abondance_capped_t-1"] = df_species["abondance_capped_t-1"]
